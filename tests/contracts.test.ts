...
import { zArtifactListInput, zArtifactPreviewImageInput } from "../src/mcp/toolSchemas.js";
...
describe("published JSON contracts", () => {
  ...
  it("keeps artifact_preview_image input contract aligned with runtime top-level parameters", async () => {
    const schema = await readJson(path.resolve("contracts/tools/artifact_preview_image.v1.schema.json"));
    const publishedKeys = Object.keys((schema.properties ?? {}) as Record<string, unknown>).sort();
    const runtimeKeys = Object.keys(zArtifactPreviewImageInput.shape).sort();
